...
# Welcome
 dialog --backtitle "MasOS Utility" --title "MasOS Retroarch Bezel Utility Menu" \
    --yesno "\nMasOS Retroarch Bezel Utility menu.\n\nThis utility will provide a quick way to hide or show bezels within Retroarch emulators.\n\nYou can use this utility to hide or show many systems at the same time or individually.\n\nFor handheld systems, there are three choices for bezels.  You can chose 1080p, 720p, or other.  Depending upon your TV/monitor resolution, you may have to try a couple of them to get the right one.\n\nIf you find none of these choices work correctly, you'll need to go into RetroArch (select+x) to adjust manually.\n\nThere is an option to install conosle game bezels. When enabled, this will show game or genre bezels for many console based games.  Enabling console bezels will automatically enable system bezels for those consoles as well.\n\n\nDo you want to proceed?" \
    30 110 2>&1 > /dev/tty \
    || exit
...
function main_menu() {
    ...
    while true; do
        choice=$(dialog --backtitle "$BACKTITLE" --title " MAIN MENU " \
            ...
            3 "Enable system bezel pack #1 (can only have 1 enabled at a time)" \
            4 "Enable system bezel pack #2 (can only have 1 enabled at a time)" \
            5 "Enable system bezel pack #3 (can only have 1 enabled at a time)" \
            6 "Optional: enable console game bezels" \
            2>&1 > /dev/tty)
